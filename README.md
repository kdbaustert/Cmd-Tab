...
| Gaps | Space left around a tiled window, set **per screen edge** — top, bottom, left and right, each with its own slider. The edges are not interchangeable in practice: a Dock on one side, a camera notch at the top, or a display whose bottom edge is level with a second monitor all want a different amount of room from the other three. A tile edge lying against the screen takes that edge's **full** gap; an edge where two windows meet takes **half the mean of its axis**, so neighbours still sit exactly one gap apart however lopsided the outer values are. All zero keeps them flush. Set every edge to the same value and the result is identical to the single gap this replaced. Applies to the keyboard arrangements and to drag-snapping, whose preview is inset to match so it shows where the window will actually land. Not applied to *Center* (it keeps the window's own size), *Restore* (it puts back a frame you chose) or the display moves. Uniform values reproduce Rectangle's `GapCalculation` arithmetic, including its consequence that a middle third is half a gap wider than the outer two. Persisted as `windowTilingGapTop`, `windowTilingGapBottom`, `windowTilingGapLeft` and `windowTilingGapRight`; a value stored under the older single `windowTilingGap` is migrated to all four on first read. | 0 (off) |
